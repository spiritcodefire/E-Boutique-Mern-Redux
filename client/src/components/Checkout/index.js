...
const Checkout = () => {
  ...
  const { first, last, email, city, country } = user ?? {}; // , gender à été enlever de la const pour supprimer l'erreur
  const {
    formValues,
    validate,
    register,
  ...
  const { delivery } = formValues["checkout"] ?? {}; // address à été enlever de la const pour supprimer l'erreur
